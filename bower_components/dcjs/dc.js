...
Usually this function is used in combination with window.d3.scale.ordinal() on x axis.
**/
...
**/
    ...
    dc.dateFormat = window.d3.time.format("%m/%d/%Y");
    ...
    dc.utils.printSingleValue.fformat = window.window.d3.format(".2f");
...
**/
    ...
    **/
      _chart.width = function (w) {
        ...
        _width = window.window.d3.functor(w || _default_width);
        return _chart;
      };
    ...
    **/
      _chart.height = function (h) {
        ...
        _height = window.window.d3.functor(h || _default_height);
        return _chart;
      };
    ...
    **/
      _chart.data = function (d) {
        ...
        _data = window.window.d3.functor(d);
        _chart.expireCache();
        return _chart;
      };
    ...
    **/
    ...
    ```js
    window.d3.select("#chart-id").select(selector);
    ```
    ...
    **/
    ...
    ```js
    window.d3.select("#chart-id").selectAll(selector);
    ```
    ...
    **/
      _chart.anchor = function (a, chartGroup) {
        ...
        if (dc.instanceOfChart(a)) {
          ...
          _anchor = a;
          _root = window.window.d3.select(_anchor);
    ...
    **/
      ...
      _chart.highlightSelected = function (e) {
        window.window.d3.select(e).classed(dc.constants.SELECTED_CLASS, true);
        window.window.d3.select(e).classed(dc.constants.DESELECTED_CLASS, false);
      };

      _chart.fadeDeselected = function (e) {
        window.window.d3.select(e).classed(dc.constants.SELECTED_CLASS, false);
        window.window.d3.select(e).classed(dc.constants.DESELECTED_CLASS, true);
      };
...
**/
    ...
    dc.colorMixin = function (_chart) {
      var _colors = window.window.d3.scale.category20c();
    ...
    `window.d3.scale.category20c()`.
    ```js
    // alternate categorical scale
    chart.colors(window.d3.scale.category20b());

    // ordinal scale
    chart.colors(window.d3.scale.ordinal().range(['red','green','blue']);
    ...
    **/
      _chart.colors = function (_) {
        ...
        if (_ instanceof Array) _colors = window.window.d3.scale.quantize().range(_); // depricated legacy support, note: this fails for ordinal domains
        else _colors = window.window.d3.functor(_);
        return _chart;
      };

      /**
    #### .ordinalColors(r)
    Convenience method to set the color scale to window.d3.scale.ordinal with range `r`.

    **/
      _chart.ordinalColors = function (r) {
        return _chart.colors(window.window.d3.scale.ordinal().range(r));
      };
    ...
    **/
      _chart.linearColors = function (r) {
        return _chart.colors(window.window.d3.scale.linear()
          .range(r)
          .interpolate(window.window.d3.interpolateHcl));
      };
    ...
    **/
      _chart.calculateColorDomain = function () {
        var newDomain = [window.window.d3.min(_chart.data(), _chart.colorAccessor()),
          window.window.d3.max(_chart.data(), _chart.colorAccessor())
        ];
...
**/
    dc.coordinateGridMixin = function (_chart) {
      ...
      _chart.colors(window.window.d3.scale.category10());
      ...
      var _xAxis = window.window.d3.svg.axis().orient("bottom");
      ...
      var _y;
      var _yAxis = window.window.d3.svg.axis().orient("left");
      var _yAxisPadding = 0;
      var _yElasticity = false;
      var _yAxisLabel;
      var _yAxisLabelPadding = 0;

      var _brush = window.window.d3.svg.brush();
      var _brushOn = true;
      var _round;
      ...
      var _zoom = window.window.d3.behavior.zoom().on("zoom", zoomHandler);
      var _nullZoom = window.window.d3.behavior.zoom().on("zoom", null);
    ...
    **/
    ...
    chart.x(window.d3.scale.linear().domain([-2500, 2500]))
    // set x to a time scale to generate histogram
    chart.x(window.d3.time.scale().domain([new Date(1985, 0, 1), new Date(2012, 11, 31)]))
    ```
    ...
    **/
    ...
    expected to return an Javascript array of all data points on x axis. d3 time range functions window.d3.time.days, window.d3.time.months,
    and window.d3.time.years are all valid xUnits function. dc.js also provides a few units function, see [Utilities](#utilities)
    ...
    chart.xUnits(window.d3.time.days);
    // set x units to month for a histogram
    chart.xUnits(window.d3.time.months);
    ```
    ...
    **/
      ...
      _chart._prepareYAxis = function (g) {
        if (_y === undefined || _chart.elasticY()) {
          _y = window.window.d3.scale.linear();
    ...
    **/
      _chart.xAxisMin = function () {
        var min = window.window.d3.min(_chart.data(), function (e) {
    ...
    **/
      _chart.xAxisMax = function () {
        var max = window.window.d3.max(_chart.data(), function (e) {
          return _chart.keyAccessor()(e);
        });
    ...
    **/
    ...
    chart.round(window.d3.time.month.round);
    ```
    ...
    **/
      ...
      function constrainRange(range, constraint) {
        ...
        constrainedRange[0] = window.window.d3.max([range[0], constraint[0]]);
        constrainedRange[1] = window.window.d3.min([range[1], constraint[1]]);
...
Stack Mixin is an mixin that provides cross-chart support of stackability using window.d3.layout.stack.

**/
    dc.stackMixin = function (_chart) {

      var _stackLayout = window.window.d3.layout.stack()
      ...
      function domainFilter() {
        if (!_chart.x()) return window.window.d3.functor(true);
        var xDomain = _chart.x().domain();
        if (_chart.isOrdinal()) {
          // TODO #416
          //var domainSet = window.d3.set(xDomain);
          return function (p) {
    ...
    **/
      ...
      _chart.yAxisMin = function () {
        var min = window.window.d3.min(flattenStack(), function (p) {
      ...
      _chart.yAxisMax = function () {
        var max = window.window.d3.max(flattenStack(), function (p) {
          return p.y + p.y0;
        });
      ...
      _chart.xAxisMin = function () {
        var min = window.window.d3.min(flattenStack(), dc.pluck('x'));
        return dc.utils.subtract(min, _chart.xAxisPadding());
      };

      _chart.xAxisMax = function () {
        var max = window.window.d3.max(flattenStack(), dc.pluck('x'));
...
**/
    dc.capMixin = function (_chart) {
      ...
      var _othersGrouper = function (topRows) {
        var topRowsSum = window.d3.sum(topRows, _chart.valueAccessor()),
          allRows = _chart.group().all(),
          allRowsSum = window.window.d3.sum(allRows, _chart.valueAccessor()),
          topKeys = topRows.map(_chart.keyAccessor()),
          allKeys = allRows.map(_chart.keyAccessor()),
          topSet = window.window.d3.set(topKeys),
...
**/
    dc.bubbleMixin = function (_chart) {
      ...
      var _r = window.window.d3.scale.linear().domain([0, 100]);
      ...
      /**
    #### .r([bubbleRadiusScale])
    Get or set bubble radius scale. By default bubble chart uses ```window.window.d3.scale.linear().domain([0, 100])``` as it's r scale .
...
**/
    ...
    **/
      ...
      function drawChart() {
        ...
        _radius = _radius ? _radius : window.window.d3.min([_chart.width(), _chart.height()]) / 2;
    ...
    **/
      ...
      function buildArcs() {
        return window.window.d3.svg.arc().outerRadius(_radius).innerRadius(_innerRadius);
      }
    ...
    **/
      ...
      function pieLayout() {
        return window.window.d3.layout.pie().sort(null).value(_chart.cappedValueAccessor);
      }
      ...
      function tweenPie(b) {
        ...
        var i = window.window.d3.interpolate(current, b);
        this._current = i(0);
        return function (t) {
     ...
     */
      ...
      function labelPosition(d, arc) {
        var centroid;
        if (_externalLabelRadius) {
          centroid = window.window.d3.svg.arc()
      ...
      function highlightSliceFromLegendable(legendable, highlighted) {
        _chart.selectAll('g.pie-slice').each(function (d) {
          if (legendable.name == d.data.key) {
            window.window.d3.select(this).classed("highlight", highlighted);
          }
        });
      }
...
**/
    dc.barChart = function (parent, chartGroup) {
      ...
      _chart.plotData = function () {
        ...
        layers.each(function (d, i) {
          var layer = window.window.d3.select(this);
    ...
    **/
    ...
    When using standard window.window.d3.js rounding methods, the brush often doesn't align correctly with centered bars since the bars are offset.
    ...
    **/
      ...
      function colorFilter(color, inv) {
        return function () {
          var item = window.window.d3.select(this);
...
**/
    ...
    **/
      ...
      function drawLine(layersEnter, layers) {
        var line = window.window.d3.svg.line()
          .x(function (d) {
            return _chart.x()(d.x);
          })
          .y(function (d) {
      ...
      function drawArea(layersEnter, layers) {
        if (_renderArea) {
          var area = window.window.d3.svg.area()
            .x(function (d) {
              return _chart.x()(d.x);
            })
            .y(function (d) {
          ...
          layers.each(function (d, layerIndex) {
            ...
            dots.enter()
              ...
              .on("mousemove", function (d) {
                var dot = window.window.d3.select(this);
                showDot(dot);
                showRefLines(dot, g);
              })
...
**/
    dc.dataCount = function (parent, chartGroup) {
      ...
      var _formatNumber = window.window.d3.format(",d");
...
**/
    dc.dataTable = function (parent, chartGroup) {
      ...
      var _order = window.window.d3.ascending;
    ...
    **/
      ...
      /**
    #### .order([order])
    Get or set sort order. Default value: ``` window.window.d3.ascending ```

    ```js
        chart.order(window.window.d3.descending);
    ```
 ...
 **/
     ...
     **/
      ...
      /**
     #### .order([order])
     Get or set sort order function. Default value: ``` window.window.d3.ascending ```
...
**/
    dc.compositeChart = function (parent, chartGroup) {
      ...
      var _rightYAxis = window.window.d3.svg.axis(),
        _rightYAxisLabel = 0,
      ...
      function prepareRightYAxis() {
        if (_chart.rightY() === undefined || _chart.elasticY()) {
          _chart.rightY(window.window.d3.scale.linear());
      ...
      function prepareLeftYAxis() {
        if (_chart.y() === undefined || _chart.elasticY()) {
          _chart.y(window.window.d3.scale.linear());
    ...
    **/
      ...
      function yAxisMin() {
        return window.d3.min(getYAxisMin(leftYAxisChildren()));
      }

      function rightYAxisMin() {
        return window.d3.min(getYAxisMin(rightYAxisChildren()));
      }
      ...
      function yAxisMax() {
        return dc.utils.add(window.d3.max(getYAxisMax(leftYAxisChildren())), _chart.yAxisPadding());
      }

      function rightYAxisMax() {
        return dc.utils.add(window.d3.max(getYAxisMax(rightYAxisChildren())), _chart.yAxisPadding());
      }
      ...
      dc.override(_chart, 'xAxisMin', function () {
        return dc.utils.subtract(window.d3.min(getAllXAxisMinFromChildCharts()), _chart.xAxisPadding());
      });
      ...
      dc.override(_chart, 'xAxisMax', function () {
        return dc.utils.add(window.d3.max(getAllXAxisMaxFromChildCharts()), _chart.xAxisPadding());
      });
 ...
 **/
    dc.seriesChart = function (parent, chartGroup) {
      ...
      var _seriesAccessor;
      var _seriesSort = window.d3.ascending;
      ...
      function keySort(a, b) {
        return window.d3.ascending(_chart.keyAccessor()(a), _chart.keyAccessor()(b));
      }

      _chart._preprocessData = function () {
        var keep = [];
        var children_changed;
        var nester = window.d3.nest().key(_seriesAccessor);
        if (_seriesSort)
        ...
        var children =
          nesting.map(function (sub, i) {
            ...
            return subChart
              ...
              .group({
                all: window.d3.functor(sub.values)
              }, sub.key)
     ...
     **/
     ...
     Example:
     ```
     chart.seriesSort(window.d3.descending);
     ```
     **/
...
**/
    dc.geoChoroplethChart = function (parent, chartGroup) {
      ...
      var _geoPath = window.d3.geo.path();
      var _projectionFlag;
      ...
      function renderPaths(regionG, layerIndex, data) {
        var paths = regionG
          .select("path")
          .attr("fill", function () {
            var currentFill = window.d3.select(this).attr("fill");
            if (currentFill)
              return currentFill;
            return "none";
          })
...
var bubbleChart1 = dc.bubbleOverlayChart("#chart-container1").svg(window.d3.select("#chart-container1 svg"));
// create a bubble overlay chart on top of "#chart-container2 svg" element using chart group A
var bubbleChart2 = dc.compositeChart("#chart-container2", "chartGroupA").svg(window.d3.select("#chart-container2 svg"));
```
...
chart.svg(window.d3.select("#chart svg"));
```
        ...
        if (flag) {
          ...
          debugG
            ...
            .on("mousemove", function () {
              var position = window.d3.mouse(debugG.node());
...
**/
    dc.rowChart = function (parent, chartGroup) {
      ...
      var _xAxis = window.d3.svg.axis().orient("bottom");
      ...
      function calculateAxisScale() {
        if (!_x || _elasticX) {
          var extent = window.d3.extent(_rowData, _chart.cappedValueAccessor);
          if (extent[0] > 0) extent[0] = 0;
          _x = window.d3.scale.linear().domain(extent)
 ...
 **/
    dc.scatterPlot = function (parent, chartGroup) {
      var _chart = dc.coordinateGridMixin({});
      var _symbol = window.d3.svg.symbol();
    ...
    **/
      ...
      _chart.legendHighlight = function (d) {
        ...
        _chart.selectAll('.chart-body path.symbol').filter(function () {
          return window.d3.select(this).attr('fill') != d.color;
      ...
      function resizeSymbolsWhere(condition, size) {
        var symbols = _chart.selectAll('.chart-body path.symbol').filter(function (d) {
          return condition(window.d3.select(this));
        });
...
**/
    dc.numberDisplay = function (parent, chartGroup) {
      ...
      var _formatNumber = window.d3.format(".2s");
      var _chart = dc.baseMixin({});
      var _html = {
        one: "",
        some: "",
    ...
    **/
      ...
      _chart._doRender = function () {
        ...
        span.transition()
          ...
          .ease('quad-out-in')
          .tween("text", function () {
            var interp = window.d3.interpolateNumber(this.lastValue || 0, newValue);
    ...
    Get or set a function to format the value for the display. By default `window.d3.format(".2s");` is used.
 ...
 **/
    dc.heatMap = function (parent, chartGroup) {
      ...
      _chart.rows = function (_) {
        ...
        rowValues.sort(window.d3.ascending);
        return window.d3.scale.ordinal().domain(rowValues.filter(uniq));
      };
      ...
      _chart.cols = function (_) {
        ...
        colValues.sort(window.d3.ascending);
        return window.d3.scale.ordinal().domain(colValues.filter(uniq));
      };
    ...
    (function () {
      ...
      window.d3.box = function () {
        ...
        function box(g) {
          g.each(function (d, i) {
            d = d.map(value).sort(window.d3.ascending);
            ...
            var outlierIndices = whiskerIndices ? window.d3.range(0, whiskerIndices[0]).concat(window.d3.range(whiskerIndices[1] + 1, n)) : window.d3.range(n);

            // Compute the new x-scale.
            var x1 = window.d3.scale.linear()
              .domain(domain && domain.call(this, d, i) || [min, max])
              .range([height, 0]);

            // Retrieve the old x-scale, if this is an update.
            var x0 = this.__chart__ || window.d3.scale.linear()
              .domain([0, Infinity])
              .range(x1.range());
        ...
        box.domain = function (x) {
          ...
          domain = x == null ? x : window.d3.functor(x);
          return box;
        };
      ...
      function boxQuartiles(d) {
        return [
          window.d3.quantile(d, .25),
          window.d3.quantile(d, .5),
          window.d3.quantile(d, .75)
        ];
      }
 ...
 **/
    dc.boxPlot = function (parent, chartGroup) {
      ...
      // default to ordinal
      _chart.x(window.d3.scale.ordinal());
     ...
     **/
      ...
      function updateBoxes(boxesG) {
        dc.transition(boxesG, _chart.transitionDuration())
          ...
          .call(_box)
          .each(function () {
            window.d3.select(this).select('rect.box').attr("fill", _chart.getColor);
          });
      }
      ...
      _chart.yAxisMin = function () {
        var min = window.d3.min(_chart.data(), function (e) {
          return window.d3.min(_chart.valueAccessor()(e));
        });
        return dc.utils.subtract(min, _chart.yAxisPadding());
      };

      _chart.yAxisMax = function () {
        var max = window.d3.max(_chart.data(), function (e) {
          return window.d3.max(_chart.valueAccessor()(e));
        });
        return dc.utils.add(max, _chart.yAxisPadding());
      };
     ...
     chart.tickFormat(window.d3.format(".2f"));
     ```
     **/
  ...
  if (typeof define === "function" && define.amd) {
    ...
    module.exports = _dc(window.d3);
  } else {
    this.dc = _dc(window.d3);
  }
})();
